...
import { SignupUserDto, SearchUsersDto } from "../dto/user.dto";
...
class UserService {
  ...
  async searchUsres(filters: SearchUsersDto) {
    ...
    if (filters.email) query.andWhere("user.email LIKE :email", { email: `%${filters.email}%` });
  ...
  async setNewPassword(email: string, fullHash: string, newPassword: string) {
    const user = await this.userRepo.findOne({
      where: { email },
    });
    if (!user) throw new NotFoundError("not found");
  ...
  async chnagePassword(id: number, oldPassword: string, newPassword: string) {
    const user = await this.userRepo.findOne({
      where: { id },
    });
    if (!user) throw new NotFoundError("not found");

    const isCorrect = await Password.compare(oldPassword, user.password);
  ...
  public modifyUserSpace(dto: SpaceSettingDto) {
    const query = dbConnection.datasource.createQueryBuilder().update(User).set({ totalSpace: dto.space });
    if (dto.userIds?.length) query.where("id IN (:...userIds)", { userIds: dto.userIds });
